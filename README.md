...
### Write protection (three layers)

Faculty write operations (submitting interest, accepting students) can trigger emails from the program account. These are protected by three layers:

1. **Cloudflare Worker proxy** (`worker/`): Write requests go through a Cloudflare Worker at `besure-api.billowing-sky-6472.workers.dev` instead of directly to Apps Script. The Worker holds the write API key as a server-side secret — it never appears in browser code.
2. **Origin check**: The Worker rejects any request without a valid `Origin` header matching `rkn2.github.io`.
3. **Cloudflare Turnstile**: An invisible CAPTCHA widget runs silently on faculty pages. Each write request must include a valid Turnstile token, which can only be generated by a real browser loading the actual site. Scripts, curl, and spoofed requests are all blocked.

### Read-only API key

Read operations (loading student lists, placements) go directly to Apps Script with a read-only API key. This key is visible in page source (`response-tracking.js`, `view/index.html`, `placements.html`) but can only read data — it cannot trigger writes or emails.
...
The Worker is deployed from `worker/` using [Wrangler](https://developers.cloudflare.com/workers/wrangler/):
...
- `WRITE_KEY` — the Apps Script write API key
- `TURNSTILE_SECRET` — the Cloudflare Turnstile secret key

Set or update secrets with `npx wrangler secret put SECRET_NAME`.

The Turnstile site key (public, in `response-tracking.js`) is configured for the domain `rkn2.github.io` with invisible mode (no user interaction). Manage the widget at [Cloudflare Dashboard > Turnstile](https://dash.cloudflare.com/?to=/:account/turnstile).
